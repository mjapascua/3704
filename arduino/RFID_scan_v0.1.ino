...
#define dirPin 32 
#define stepPin 33
#define manOpenPin 34
#define manClosePin 35
...
void setup() {  
  ...
  pinMode(manOpenPin, INPUT);
  pinMode(manClosePin, INPUT);
...
void loop() {
  ...
  bool rfDisabled = digitalRead(RFdisabled);
  
  if(!rfFunctional || rfDisabled){
    digitalWrite(failPin,HIGH);
  }  digitalWrite(failPin,LOW);

  
  if(digitalRead(manOpenPin)){
   digitalWrite(dirPin, HIGH);
   P.displayZoneText(1,"P A S S",PA_CENTER,P.getSpeed(), P.getPause(), PA_PRINT, PA_CLOSING);
   for (int i = 0; i < stepsPerRevolution; i++) {
      if(digitalRead(manClosePin)){
